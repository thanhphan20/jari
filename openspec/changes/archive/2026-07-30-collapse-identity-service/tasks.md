...
- [x] 8.2 Verify a direct call to a service port, bypassing the gateway, is rejected when it carries no identity headers. Then verify the harder case: a direct call that supplies a *forged* `X-Jari-User-Id` header succeeds against `IdentityHeaderFilter` (it only checks presence, not provenance) - confirm this is the accepted, documented risk (see 10.3/10.4), not a silent gap.
  - `curl http://localhost:8083/projects` directly (no gateway, no headers) -> 401.
  - `curl http://localhost:8083/projects -H "X-Jari-User-Id: 999" -H "X-Jari-Username: forged-user"` directly (no gateway, forged headers) -> 200. Demonstrates the filter accepts any caller-supplied identity header at face value; the only reason this is safe is that these ports are not meant to be reachable outside the gateway (10.3/10.4).
